...
let getHotelCard = (singleHotelData) => {
  ...
  if (a !== null) {
    var favIconClass = !a.find((hotel) => hotel.id == singleHotelData.id)
      ? "fa-heart"
      : "fa-heart-red";
  } else {
...
function searchResult() {
  ...
  let searchMatchingHotels = allHotelsDataCopy.filter(
    (hotel) =>
      hotel.tags.toString().toUpperCase().indexOf(d.value.toUpperCase()) > -1
  );
  mainRow.innerHTML = generateView(searchMatchingHotels).join("");
  errorMessageElement.innerText =
    searchMatchingHotels == 0 ? `No hotels Found for ${d.value}` : "";
}
...
selectSorting.addEventListener("click", (e) => {
  if (e.target.innerText === "Sort by ETA") {
    let sortByEta = allHotelsDataCopy.sort(
      (hotel1, hotel2) => hotel1.eta - hotel2.eta
    );
    mainRow.innerHTML = generateView(sortByEta).join("");
  } else if (e.target.innerText === "Sort by Rating") {
    let sortByRating = allHotelsDataCopy.sort(
      (hotel1, hotel2) => hotel2.rating - hotel1.rating
    );
...
selectFilter.addEventListener("click", (e) => {
  let filteredHotels = allHotelsDataCopy.filter(
    (hotel) =>
      hotel.tags
        .toString()
        .toUpperCase()
        .indexOf(e.target.innerText.toUpperCase()) > -1
  );
  mainRow.innerHTML = generateView(filteredHotels).join("");
  errorMessageElement.innerText =
    filteredHotels.length == 0
      ? `No Restro belong to ${e.target.innerText}`
      : "";
});
